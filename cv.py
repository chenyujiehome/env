...
# %% [markdown]
# The logits are of shape `(ba
# tch_size, num_queries, number of classes + 1)`. We model internally adds an additional "no object class", which explains why we have one additional output for the class dimension. 
...
trainer = Trainer( max_steps=10, gradient_clip_val=0.1, accumulate_grad_batches=4)
trainer.fit(model)
